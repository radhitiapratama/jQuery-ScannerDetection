 ...
 * 001	1/10/2018	Various modifications to support both keyboard input and scanning.  preventDefault has been redefined such that when set true,
 *							only scanner input is suppressed from appearing in input fields.  Previously, setting preventDefault to true suppressed all
 *							input, both typed in and scanner.
 * 
 */
(function($){
				...
				$self.data('scannerDetection',{options:options}).unbind('.scannerDetection').bind('keydown.scannerDetection',function(e){
				 ...
				 || (!firstCharTime && options.startChar.indexOf(e.which)!==-1)){
				  ...
				  if (options.stopPropagation) e.stopImmediatePropagation();
					...
					// 001 
					//	Prevent default key handling only when scanning. 
					 if (firstCharTime && options.preventDefault) e.preventDefault();  
					 ...
					 if(firstCharTime && options.endChar.indexOf(e.which)!==-1){
						...
						// 001
					 	// firstCharTime was being set unconditionally below.  changed this so that only
					 	// set when the prefix character is encountered  
						  firstCharTime = Date.now();
	
					 } else {
						// 001
						// Only add to the scanned string if the scan prefix has been encountered. 
						  if (firstCharTime && (typeof(e.which) != 'undefined')){
								stringWriting+=String.fromCharCode(e.which);
						 }
						 callIsScanner=false;
					 }
					//  001
					//  FirstCharTime now set above only when the prefix character is encountered.  this resolves issues
					//  with typed-in characters being discarded when typed rapidly   
					 //if(!firstCharTime){
					 //    firstCharTime=Date.now();
					 //}
					 ...
					 if(callIsScanner){
					 	...
					 	testTimer=false;
					// 001
					// only set timer after scan prefix has been detected. 
					 //} else {
					 } else if (firstCharTime) {		
